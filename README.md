...
- `src/lib/config.ts`: Application configuration including default audio URLs and key terms
- `src/lib/models.ts`: TypeScript type definitions for database entities, queues, and API responses

### API Routes (Main System Components)

- `src/app/api/migration-router/route.ts`: Routes incoming calls between existing and Deepgram systems
- `src/app/api/dg-submission-worker/route.ts`: Handles submission of audio files to Deepgram API
- `src/app/api/dg-webhook/route.ts`: Receives and processes Deepgram transcription responses
- `src/app/api/dg-processing-worker/route.ts`: Post-processes Deepgram results for integration
- `src/app/api/observability/route.ts`: System monitoring and health check endpoints

### Mock System Components

- `src/mock-system/database.ts`: In-memory database simulation for development
- `src/mock-system/dg-submission-queue.ts`: Mock queue for managing Deepgram submissions
- `src/mock-system/dg-processing-queue.ts`: Mock queue for post-processing tasks

### React Components

- `src/app/page.tsx`: Main dashboard interface
- `src/components/EventControls.tsx`: Control buttons for triggering system events
- `src/components/SupportCallDatabase.tsx`: Display and management of support call records
- `src/components/SystemState.tsx`: Real-time system status and queue monitoring
